...
function App() {
        ...
        <div className="hero flex flex-column">
            <div>
                <div className="welcomeText">
                    Video Calls & Sharing Screen
                </div>
                <div className="descriptionText">
                   anywhere in the world for free using Gitsalahcall
                </div>
            </div>
            <div>
                <div className="actionText">Do u want to call someone ? this is your Nickname <span className={copied?"username highlight copied":"username highlight"} onClick={()=>{showCopiedMessage()}}>{yourID}</span>?</div>
            </div>
            <div className="callBox flex">
                <input type="text" placeholder="Friend Nickname" value={receiverID} onChange={e => setReceiverID(e.target.value)} className="form-input"/>
                <button onClick={() => callPeer(receiverID.toLowerCase().trim())} className="primaryButton">Call</button>
            </div>
            <div>
                To call your friend, ask them to open gitsalahcall in other browser or device. <br/>
                Send your username (<span className="username">{yourID}</span>) and wait for the call <span style={{fontWeight: 600}}>OR</span> enter the username and hit call!
            </div>
        </div>
    </div>
    </div>
  ...
  function callPeer(id) {
    if(id!=='' && users[id] && id!==yourID){
            ...
            iceServers: [
                {
                    ...
                    username: "salah.bouanba2@gmail.com",
                    credential: "0646274243"
                },
                {
      ...
      setModalMessage('Maybe the username entered is wrong. Please check again and retry!')
      setModalVisible(true)
      return
    }
  }
    ...
    incomingCall = (
      <div className="incomingCallContainer">
        <div className="incomingCall flex flex-column">
          ...
          <button name="accept" className="acceptCall" onClick={()=>acceptCall()}>Accept</button>
          <button name="reject" className="rejectCall" onClick={()=>rejectCall()}>Reject</button>
          </div>
        </div>
      </div>
    )
